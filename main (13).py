...
def health():
    ...
    return {"status":"ok","initialized":store["initialized"],
            ...
            "market_opens":"09:15 IST","market_closes":"15:30 IST",
            "5y_highs_loaded":len(store["fivey_highs"])}
...
def get_chart_data(symbol: str, period: str):
    ...
    try:
        ...
        if df.empty: raise HTTPException(status_code=404, detail="No data — market may not have opened yet (NSE: 9:15–15:30 IST)")
        ...
        if df.empty: raise HTTPException(status_code=404, detail="No intraday data yet — NSE opens at 9:15 IST")
        skip_ma = cfg["interval"] in ("1wk","1mo")
        if len(df)>5 and not skip_ma:
            df["SMA5"]  = df["Close"].rolling(5).mean()
            df["EMA13"] = df["Close"].ewm(span=13,adjust=False).mean()
            df["EMA26"] = df["Close"].ewm(span=26,adjust=False).mean()
            df["RSI14"] = compute_rsi(df["Close"],14)
        elif len(df)>14:
            # For 5Y/MAX still compute RSI (useful, works on weekly data)
            df["RSI14"] = compute_rsi(df["Close"],14)
